...
##----------------------------------------------------------**----------------------------------------------------------##
    plot(t, value, linewidth=1.0)
    xlabel('DateTime')
    ylabel(selected_variable.rstrip() + " (" + unitname + ")")
#    title('A simple graph ')
    ...
    args['img'] = str((buffer.getvalue()).encode('Base64'))
    args['timestep'] = "raw"
...
##----------------------------------------------------------**----------------------------------------------------------##   
    ...
    if str(time_step) != "raw":
        ...
        resample_data = data_object.resample_data(time_step)
        graph_img = serial_statistics.graph_in_endcode64(resample_data,selected_variable.rstrip(),unitname,linear_regression)
        value = resample_data.values
    else:
        data_object = serial_statistics.serial_statistics(t,value,VariableType)
        graph_img = serial_statistics.graph_in_endcode64(data_object.DF,selected_variable.rstrip(),unitname,linear_regression)
...
## Resample data 
    if VariableType == 'sum':
        data_object = serial_statistics.serial_statistics(t,value,VariableType)
        resample_DF = (data_object.resample_data('M')).values
...
def extract_data(request):
    ...
    if request.user.is_staff:
        ...
        selected_endtime = request.POST.get('endtime')
        time_step = request.POST.get('timestep')
        remove_outliers_choices = request.POST.get('removeoutlier')
        filldata = request.POST.get('filldata')
...
def get_data(id_list,varibale_list,starttime_list,endtime_list):
    ...
    return variable_header,variable_type, final_data, unitname, unit_abbr, SiteID_LatLong_lib, varibalename_variablecode_dict
...
def validation_analysis(request):
    ...
    variable_header,variable_type, ob_data, unitname, unit_abbr, SiteID_LatLong_lib, varibalename_variablecode_dict = get_data([selected_id],[selected_variable],[selected_starttime],[selected_endtime])
    print "test2"
...
def general_multivariable_statistic(request):
    ...
    variable_header,variable_type, datavalue, unitname, unit_abbr, SiteID_LatLong_lib, variablecode_dict = get_data(selected_id,selected_variable,selected_starttime,selected_endtime)
...
#    user_endtime = request.POST.get('user_endtime')
    ...
    variable_header,variable_type, datavalue, unitname, unit_abbr, SiteID_LatLong_lib, varibalename_variablecode_dict = get_data(selected_id,selected_variable,selected_starttime,selected_endtime)
