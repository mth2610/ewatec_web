...
def importseriesdata(request):
    ...
    return render_to_response('importseriesdata.html',context_instance=RequestContext(request))
...
def fill_QualityControlLevel(data):
    if data == '' or data == 'NULL' or data == 'Null' or data == 'null':
        return -9999
    else: 
        return data
    
def fill_CensorCode(data):
    if data == '' or data == 'NULL' or data == 'Null' or data == 'null':
        return 'nc'
    else:
        return data
...
class import_datavalues():
    ...
    def importdata(self):
        ...
        ## Fill data for CensorCode and QualityControlLevelID
        for i in range(n):
            CensorCode[i] = fill_CensorCode(CensorCode[i])
            QualityControlLevelID[i] = fill_QualityControlLevel(QualityControlLevelID[i])
...
#                else:
        ...
        for i in range(n):
            connection.rollback()
            starttime_data = connection.cursor()
            starttime_str = 'SELECT MIN("LocalDateTime")'\
                           + ' FROM dbo."DataValues"'\
                           + ' WHERE '+ '"SiteID" = ' + str(format_data(SiteID_SiteCode_lib[(index.id_code)[i]]))  + " AND "\
                           + '"VariableID" =' + str(format_data(VariableID_VariableCode_lib[(index.variable_code)[i]]))
            starttime_data.execute(starttime_str)
            connection.commit()
            starttime_data =  ((starttime_data.fetchall())[0])[0]
            starttime_data_str = starttime_data.strftime("%Y-%m-%d %H:%M:%S")
            
    
    ###
            endtime_data = connection.cursor()
            endtime_str = 'SELECT MAX("LocalDateTime")'\
                           + ' FROM dbo."DataValues"'\
                           + ' WHERE '+ '"SiteID" = ' + str(format_data(SiteID_SiteCode_lib[(index.id_code)[i]]))  + " AND "\
                           + '"VariableID" =' + str(format_data(VariableID_VariableCode_lib[(index.variable_code)[i]]))
            endtime_data.execute(endtime_str)
            connection.commit()
            endtime_data = ((endtime_data.fetchall())[0])[0]
            endtime_data_str = endtime_data.strftime("%Y-%m-%d %H:%M:%S")
            
            
            
            count_data = connection.cursor()
            count_str = 'SELECT COUNT("LocalDateTime")'\
                           + ' FROM dbo."DataValues"'\
                           + ' WHERE '+ '"SiteID" = ' + str(format_data(SiteID_SiteCode_lib[(index.id_code)[i]]))  + " AND "\
                           + '"VariableID" =' + str(format_data(VariableID_VariableCode_lib[(index.variable_code)[i]]))
            count_data.execute(count_str)
            connection.commit()
            count_data= ((count_data.fetchall())[0])[0]   
            # 
            chec_existingdata = connection.cursor()
            check_existingdata_str = 'SELECT "SiteID" FROM dbo."SeriesCatalog"'+ ' WHERE '+ '"SiteCode" = ' + "'" + (index.id_code)[i]+ "'" + " AND "\
                                  + '"VariableCode" ='+ format_data((index.variable_code)[i])
            chec_existingdata.execute(check_existingdata_str)
            connection.commit()
            chec_existingdata = chec_existingdata.fetchall()

            
            if chec_existingdata != []:
                update_data = connection.cursor()
                update_str = 'UPDATE dbo."SeriesCatalog" SET "BeginDateTime" = ' + format_data(starttime_data_str) + ","\
                            + '"EndDateTime" = ' + format_data(endtime_data_str) + ","\
                            + '"BeginDateTimeUTC" = ' + format_data((starttime_data+UTC_OFFSET_TIMEDELTA).strftime("%Y-%m-%d %H:%M:%S")) + ","\
                            + '"EndDateTimeUTC" = ' + format_data((endtime_data+UTC_OFFSET_TIMEDELTA).strftime("%Y-%m-%d %H:%M:%S"))\
...
##                ## Get source data
               ...
               ## Insert data into serierscatalog
                ...
                insert_str = 'INSERT INTO dbo."SeriesCatalog" ("SiteID","SiteCode","SiteName","SiteType",'\
                              ...
                              + format_data(starttime_data_str) + ","\
                              + format_data(endtime_data_str) + ","\
                              + format_data((starttime_data+UTC_OFFSET_TIMEDELTA).strftime("%Y-%m-%d %H:%M:%S")) + ","\
                              + format_data((endtime_data+UTC_OFFSET_TIMEDELTA).strftime("%Y-%m-%d %H:%M:%S")) + ","\
                              + format_data(count_data)+')'
...
#                connection.commit()
#            except ValueError:
#                print "Your file was not in correct formant. Please check it again! "
    ...
    def importdata(self):
        ...
        for i in range(n):
            ...
            insert_str = 'INSERT INTO dbo."Variables" ("VariableCode","VariableName","Speciation","VariableUnitsID","SampleMedium","ValueType","IsRegular","TimeSupport","TimeUnitsID","DataType","GeneralCategory","NoDataValue") VALUES ('\
                    ...
                    +  str(format_data(VariableUnitsID_VariableUnitsName_lib[(VariableUnitsName[i]).rstrip()])) + ","\
